    ...
    /***/ (__unused_webpack_module, exports, __webpack_require__) => {
      ...
      const PDFViewerApplication = {
        ...
        bindEvents() {
          ...
          // 处理错误
          eventBus._on('documenterror', (error) => {
            window.alert('打开文件失败')
            // window.parent.close()
          })
      ...
      function webViewerPageRendered({ source, pageNumber, error }) {
        ...
        const page = source.div
        const WatermarkPlus = window.parent?.WatermarkPlus
        if (WatermarkPlus) {
          const watermark = new WatermarkPlus.Watermark({
            width: page.clientWidth / 4,
            // height: page.clientHeight / 5,
            content: window.parent?.email,
            parent: page,
            globalAlpha: 0.3,
            zIndex: 99,
            fontSize: '14px',
